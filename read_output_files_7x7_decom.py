...
solve_time = []
total_solve_time = []
agents = np.zeros(20) #10 timesteps
num_agents = []
...
f = open('macc_outputs/txt_files_6structs.csv', 'w')
header = ['Name', 'T', 'No. of agents', 'Solve time', 'Total solve time']
...
for i in range(len(T)):
    ...
    writer.writerow([name[i],T[i], num_agents[i], solve_time[i], total_solve_time[i]])
        # {'Name' : name[i],
                     # 'T': T[i],
